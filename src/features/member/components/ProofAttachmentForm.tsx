...
const ProofAttachmentForm: React.FC<ProofAttachmentFormProps> = ({ onUpload }) => {
  ...
  const handleSave = async () => {
    if (!fileToUpload) {
      toast.warning("Por favor, selecione um arquivo para anexar.");
      return;
    }
    ...
    try {
      ...
      toast.success(`Comprovante "${fileToUpload.name}" enviado com sucesso!`);

      setFileToUpload(null);
    } catch (error) {
      console.error("Erro ao fazer upload:", error);
      toast.error("Ocorreu um erro ao tentar anexar o comprovante.");
    } finally {
      setIsLoading(false);
    }
  };
    ...
    <div className="bg-white p-4 rounded-lg shadow-xl lg:ml-30 lg:mr-30 ">
      ...
      <label className="relative block w-full h-32 border-2 border-dashed border-gray-300 rounded-lg cursor-pointer hover:border-blue-400 transition-colors ">
        ...
        <button
          ...
          className={`absolute  bottom-2 right-2 px-3 py-1 text-white text-xs rounded-lg font-bold transition-opacity duration-300 ${!fileToUpload || isLoading
